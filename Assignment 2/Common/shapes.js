	...
	shape.prototype.update_uniforms = function( graphicsState, model_transform, material )			// Send the current matrices to the shader
		{
				...
				lightPositions.push( vec4( 0, 10 * Math.cos(graphicsState.animation_time/1000), 10 * Math.sin(graphicsState.animation_time/1000), 1 ) );    
				lightColors.push( vec4( 0, 0, 0, .5) );   
...
function my_cube(points_transform)
	{
		shape.call(this);
		this.populate( this, mat4() );
		this.init_buffers();
	}
inherit(my_cube, shape);
	
	my_cube.prototype.populate = function( recipient, points_transform)
	{
		var offset = recipient.vertices.length;		
		var index_offset = recipient.indices.length;				
				
        recipient.vertices.push( vec3(0.5, 0.5, 0.5), vec3(0.5, -0.5, 0.5), vec3(-0.5, -0.5, 0.5) );
        recipient.vertices.push( vec3(0.5, 0.5, 0.5), vec3(-0.5, -0.5, 0.5), vec3(-0.5, 0.5, 0.5) );
	    ...
	    recipient.texture_coords.push( vec2(1/4,2/3), vec2(1/4,1/3), vec2(0,1/3) ); 
		recipient.texture_coords.push( vec2(1/4,2/3), vec2(0,1/3), vec2(0,2/3) ); 
		recipient.texture_coords.push( vec2(3/4,2/3), vec2(3/4,1/3), vec2(2/4,1/3) ); 
		recipient.texture_coords.push( vec2(3/4,2/3), vec2(2/4,1/3), vec2(2/4,2/3) );
		recipient.texture_coords.push( vec2(1, 1), vec2(1, 2/3), vec2(0, 2/3));
		recipient.texture_coords.push( vec2(1, 1), vec2(0, 2/3), vec2(0, 1));
		recipient.texture_coords.push( vec2(1, 1/3), vec2(1, 0), vec2(0, 0));
		recipient.texture_coords.push( vec2(1, 1/3), vec2(0, 0), vec2(0, 1/3));
		recipient.texture_coords.push( vec2(1/2, 2/3), vec2(1/2, 1/3), vec2(1/4, 1/3));
		recipient.texture_coords.push( vec2(1/2, 2/3), vec2(1/4, 1/3), vec2(1/4, 2/3));
		recipient.texture_coords.push( vec2(1, 2/3), vec2(1, 1/3), vec2(3/4, 1/3));
		recipient.texture_coords.push( vec2(1, 2/3), vec2(3/4, 1/3), vec2(3/4, 2/3));


		for (var i = 0; i < recipient.vertices.length - offset; i++)
...
inherit(pyramid, shape);
	...
	pyramid.prototype.populate = function( recipient, points_transform)
	{
		...
		recipient.vertices.push(vec3(0, 0.5, 0), vec3(-0.5, -0.5, 0.5), vec3(0.5, -0.5, 0.5));
		recipient.vertices.push(vec3(0, 0.5, 0), vec3(0.5, -0.5, 0.5), vec3(0.5, -0.5, -0.5));
		recipient.vertices.push(vec3(0, 0.5, 0), vec3(-0.5, -0.5, -0.5), vec3(0.5, -0.5, -0.5));
		recipient.vertices.push(vec3(0, 0.5, 0), vec3(-0.5, -0.5, -0.5), vec3(-0.5, -0.5, 0.5));


		recipient.texture_coords.push( vec2(1,1), vec2(1,0), vec2(0,0) );
		recipient.texture_coords.push( vec2(1,1), vec2(0,0), vec2(0,1) ); 
		recipient.texture_coords.push( vec2(1/2,1/2), vec2(0,0), vec2(0,1) );
		recipient.texture_coords.push( vec2(1/2,1/2), vec2(0,0), vec2(0,1) );
		recipient.texture_coords.push( vec2(1/2,1/2), vec2(0,0), vec2(0,1) );
		recipient.texture_coords.push( vec2(1/2,1/2), vec2(0,0), vec2(0,1) );
...
function capped_cylinder( num_tris, points_transform )		// Arrange triangles in a fan.  This version goes all the way around a circle with them.
	{	
		shape.call(this);	
		...
		this.createCircleVertices = function( recipient, num_tris ) 
			{	
				...
				{
						recipient.vertices.push( vec3(Math.cos(2 * Math.PI * counter/num_tris), Math.sin(2 * Math.PI * counter/num_tris), -1 ) );		
						recipient.texture_coords.push( vec2( counter/num_tris, 1/3 ) );	
				}
			}
		...
		this.createTopCircleVertices = function(recipient, num_tris)
		{
			for (var counter = 0; counter++ <= num_tris;) {
				recipient.vertices.push( vec3(Math.cos(2* Math.PI * counter/num_tris), Math.sin(2 * Math.PI * counter/num_tris), 1 ) );
				recipient.texture_coords.push( vec2( counter/num_tris, 2/3 ) );	
			}
		}
		...
		this.initFromSequence = function( recipient, center_idx, num_tris, offset )
			{	
				...
				recipient.texture_coords.push(vec2(1/2, 5/6));
				var top_center_idx = recipient.vertices.length - 1;

				recipient.vertices.push(vec3(0,0,-1));
				recipient.texture_coords.push(vec2(1/2, 1/6));
				var bot_center_idx = recipient.vertices.length - 1;

				for(var index = offset; index <= offset + num_tris - 1;	)
				{
					recipient.indices.push( index );
					...
					recipient.indices.push(index);
					recipient.indices.push(bot_center_idx);
					recipient.indices.push(index + 1);

					recipient.indices.push(index + num_tris + 1);
					recipient.indices.push(top_center_idx);
					recipient.indices.push(index + num_tris + 2);

					recipient.indices.push(index + num_tris + 1);
					recipient.indices.push(++index);
					recipient.indices.push(index + num_tris + 1);
				}
				recipient.indices.push( offset + num_tris );
				recipient.indices.push( offset + 2*num_tris + 1);
				recipient.indices.push( offset + num_tris + 1 );	
...
inherit(capped_cylinder, shape);
	...
	capped_cylinder.prototype.populate = function( recipient, num_tris, points_transform, center_idx )
		{
			var offset = recipient.vertices.length;
			var index_offset = recipient.indices.length;		
...
function weighted_capped_cylinder( ratio, num_tris, points_transform )		// Arrange triangles in a fan.  This version goes all the way around a circle with them.
	{	
		shape.call(this);	
		...
		this.createCircleVertices = function( recipient, num_tris ) 
			{	
				for( var counter = 0; counter++ <= num_tris;   )
				{
						recipient.vertices.push( vec3(ratio * Math.cos(2 * Math.PI * counter/num_tris), ratio * Math.sin(2 * Math.PI * counter/num_tris), -1 ) );		
						recipient.texture_coords.push( vec2( counter/num_tris, 1/3 ) );	
				}
			}
...
inherit(weighted_capped_cylinder, shape);

	weighted_capped_cylinder.prototype.populate = function( recipient, num_tris, points_transform, center_idx )
		{
			var offset = recipient.vertices.length;
			var index_offset = recipient.indices.length;		
